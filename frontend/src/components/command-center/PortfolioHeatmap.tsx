...
export function PortfolioHeatmap({ rows }: { rows: HeatmapRow[] }) {
  ...
  return (
    <section className="sh-terminal-panel overflow-hidden sh-animate-in sh-stagger-3">
      <div className="flex items-start justify-between gap-2 border-b border-slate-200/80 px-3 py-2 dark:border-slate-700/80">
        <div className="min-w-0">
          ...
          <p className="text-[11px] sh-subtext">Heat-ranked · click hotel for drilldown</p>
        </div>
        <button
          type="button"
          onClick={() => {
            setSortKey("variance_vs_budget");
            setSortAsc((asc) => !asc);
          }}
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border bg-white transition hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800 ${
            sortKey === "variance_vs_budget"
              ? "border-indigo-400 text-indigo-600 dark:border-indigo-500 dark:text-indigo-400"
              : "border-slate-300 text-slate-600 dark:border-slate-600 dark:text-slate-300"
          }`}
          aria-label={
            sortAsc ? "Sort by vs budget: best performers first" : "Sort by vs budget: worst performers first"
          }
          title={
            sortAsc
              ? "Worst vs budget first — tap for best first"
              : "Best vs budget first — tap for worst first"
          }
        >
          {sortAsc ? (
